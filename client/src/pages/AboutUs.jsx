...
class AboutUs extends Component {
    ...
    render() {
      console.log('submit file', this.state)
        ...
        return (
            <div className="aboutus aboutus__wrapper">
              <div className="aboutus__header form-group">
                ...
                <hr />
                <input
                    className="aboutus__input-text form-control"
                ...
                />
                {error && (
                  <div className="aboutus__label-bold">{error}</div>
                )}
                <button className="aboutus__button btn btn-primary" id="uploadButton" onClick={this.handleUpload} ref={this.uploadButtonRef} disabled={isLoading || !model ? true : selectedFile && !uploadedFile ? false : true} >Upload!</button>
                <div className="aboutus__label">{'When the file is uploaded, you can Process/Count it.'}</div>

                <button className="aboutus__button btn btn-success" id="processVideoRoiButton" onClick={this.handleVideoRoiProcess} disabled={isLoading || !model || total_fish !== null || type === 'image' ? true : uploadedFile ? false : true} >ROI Video - Count Fish!</button>
                <button className="aboutus__button btn btn-success" id="processVideoButton" onClick={this.handleVideoProcess} disabled={isLoading || !model || total_fish !== null || type === 'image' ? true : uploadedFile ? false : true} >Video - Count Fish!</button>

                <button className="aboutus__button btn btn-success" id="processWebcamButton" onClick={this.handleWebcamProcess} disabled={isLoading || !model || total_fish !== null || type === 'image' || type === 'video' ? true : uploadedFile ? false : true} >Webcam - Count Fish!</button>
                <button className="aboutus__button btn btn-success" id="processPictureButton" onClick={this.handlePictureProcess} disabled={isLoading || !model || total_fish !== null || type === 'video' ? true : uploadedFile ? false : true} >Picture - Count Fish!</button>

                <button className="aboutus__button btn btn-danger" id="processButton" onClick={this.handleCancel} disabled={isLoading || !model} >{total_fish !== null ? 'Another File' : 'Cancel'}</button>
              </div>
              <div className="aboutus__footer form-group bg-white">
                {isLoading ?
                  (
                    <h3>Counting Fish...</h3>
                  ) : (
