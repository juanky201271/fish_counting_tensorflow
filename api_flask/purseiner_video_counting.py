...
def purseiner_video_counting_process(url_input_video, model, type, width_cms, width_pxs_x_cm):
    ...
    if (type == 'frozen_inference_graph' and version == '1'):
        r = video_fish_counting_api.object_counting_fig(url_input_video, detection_graph, category_index, is_color_recognition_enabled, width_cms, width_pxs_x_cm)

    if (type == 'saved_model_root' or type == 'saved_model_dir'):
        if (version.split('.')[0] == '1'):
            r = video_fish_counting_api.object_counting_sm(url_input_video, detection_graph, category_index, is_color_recognition_enabled, width_cms, width_pxs_x_cm)
        if (version.split('.')[0] == '2'):
            r = video_fish_counting_api_v2.object_counting_sm(url_input_video, detection_graph, category_index, is_color_recognition_enabled, width_cms, width_pxs_x_cm)

    if (type == 'ckpt_dir' and version == '2'):
        r = video_fish_counting_api_v2.object_counting_c(url_input_video, detection_graph, category_index, is_color_recognition_enabled, width_cms, width_pxs_x_cm)
